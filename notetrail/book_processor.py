...
from utils.utils import BookInfo, RuntimeConfig, markdown_render


class BookProcessor:
    IGNORE_FILES = ["CONTRIBUTING.md", "CODE_OF_CONDUCT.md", "SUMMARY.md"]
    ...
    def generate_properties(self, file_path, raw_title=None):
        return {"properties": self.generate_title_property(file_path, raw_title)}
    ...
    def dir_processor(self, dir_path: dict, root_page_id: str):
        ...
        for file_path in files_mapper:
            ...
            # create a blank page
            response = notion_client.create_page(
                parent={"page_id": root_page_id}, properties=self.generate_properties(dir_path["path"])["properties"]
            )
    ...
    def file_processor(self, file_path, page_id):
        ...
        try:
            ...
            response = notion_client.create_page(
                parent={"page_id": page_id}, properties=properties["properties"], children=children
            )
            sf = SuffixRender()
